...
class ResModule1(nn.Module):
	def __init__(self, featureshape, nclasses, clname, apc, initialization):
		...
		self.blocks = nn.ModuleList([ResBlock(True, 64, 64, 1, True) if i == 0 else ResBlock(False, 4 * 64, 64) for i in range(3)])
...
class ResModule2(nn.Module):
	def __init__(self, featureshape, nclasses, clname, apc, initialization):
		...
		self.blocks = nn.ModuleList([ResBlock(False, 256, 128, 2, True) if i == 0 else ResBlock(False, 4 * 128, 128) for i in range(4)])
		self.blocks.apply(initialization)
		self.classifier = create_classifier(clname, nclasses, featureshape, apc)
		self.classifier.apply(initialization)
	def forward_conv(self, x):
...
class ResModule3(nn.Module):
	def __init__(self, featureshape, nclasses, clname, apc, initialization):
		...
		self.blocks = nn.ModuleList([ResBlock(False, 512, 256, 2, True) if i == 0 else ResBlock(False, 4 * 256, 256) for i in range(6)])
		self.blocks.apply(initialization)
		self.classifier = create_classifier(clname, nclasses, featureshape, apc)
		self.classifier.apply(initialization)
	def forward_conv(self, x):
...
class ResModule4(nn.Module):
	def __init__(self, featureshape, nclasses, clname, apc, initialization):
		...
		self.blocks = nn.ModuleList([ResBlock(False, 1024, 512, 2, True) if i == 0 else ResBlock(False, 4 * 512, 512) for i in range(3)])
		self.blocks.apply(initialization)
...
def experiment(batchsize, traintype, clname, initname, initgain, optimizer, labelsmoothing, learningrate, learningratedecay, beta1, beta2, transport,
	...
	if experiments and nepochs > 1:
		...
		expname = ['log', 'res50-4mod-timgnet-b', dataset, traintype, d[traintype], clname, initname, f'ing{initgain}', optimizer, f'lrt{learningrate}', 
	...
	print('experiment from res50-4mod-timgnet-b.py with parameters')
	for name in names:
...
def experiments(parameters, average):
	...
	sep = '-' * f 
	print('\n' + sep, 'res50-4mod-timgnet-b.py')
	print(sep, nexperiments, 'res50-4mod-timgnet-b experiments ' + ('to average ' if average else '') + 'over parameters:')
	pprint.pprint(parameters, width = f, compact = True)
	for params in product([values for name, values in parameters]) :
		j += 1
		print('\n' + sep, 'res50-4mod-timgnet-b experiment %d/%d with parameters:' % (j, nexperiments))
		...
		print(sep, 'res50-4mod-timgnet-b experiment %d/%d over. took %.1f s. total %.1f s' % (j, nexperiments, t1, time.time() - t0))
	if average:
		acc = np.mean(accs)
